...
벽에는 실제 충돌 판정이 있다(`three/collision.js`) — 렌더링에 쓰는 것과 같은
벽 좌표(`three/wallLayout.js`)로 원기둥(플레이어)-사각형(벽) 충돌을 매 프레임
풀어서 문 없는 벽을 뚫고 지나갈 수 없다. 얇은 벽을 순간이동하듯 통과하는 걸
막기 위해 프레임당 이동 거리를 짧게 잘라놓았다(`useMovement.js`의 dt 클램프).
